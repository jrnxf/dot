...
local function mapper(mode, input, exec, opts)
  ...
  if input == '<F9>' or input == 'H' or input == 'G' then
    print('mode: ' .. mode .. ' input: ' .. input .. ' exec: ' .. exec .. ' opts:' .. vim.inspect(opts))
  end
  -- vim.api.nvim_set_keymap("n", "<leader>yyy", function() print('yy') end, {})
  -- put({ mode, input, exec, opts })
