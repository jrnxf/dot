...
M.vec_union = function(...)
  ...
  for i = 1, #args do
    if args[i] ~= nil and type(args[i]) ~= 'boolean' then
      ...
      elseif not seen[args[i]] then
        -- this argument isn't a table, but it also hasn't been seen
        -- so add it to the result. This logic is questionable in my
        -- opinion, but will leave for now. I'd almost rather force
        -- the caller of this function to make sure all provided args
        -- are array-like tables
        seen[args[i]] = true
...
M.tbl_union_extend_or_overwrite = function(leftmost, ...)
  local res = {}
